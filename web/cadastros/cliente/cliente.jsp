            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <img alt="imagem" class="" name="foto" id="foto" width="85" height="100">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control" type="text" name="idcliente" id="idcliente" value="" readonly="readonly"/>
                            ...
                            <div class="col-sm">
                                ...
                                <select class="form-control" name="idestado" id="idestado" onchange="BuscarCidadesPorEstado()" required>
                                    ...
                                    <c:forEach var="estado" items="${estados}">
                                        <option value="${estado.idEstado}" ${cliente.cidade.estado.idEstado == estado.idEstado ? "selected" : ""}>
                                            ${estado.nomeEstado}
                                        </option>
                                    </c:forEach>
                                </select>
                            ...
                            <div class="col-sm">
                                ...
                                <select class="form-control" name="idcidade" id="idcidade" required>
                                    ...
                                    <c:forEach var="cidade" items="${cidades}">
                                        <option value="${cidade.idCidade}" ${cliente.cidade.idCidade == cidade.idCidade ? "selected" : ""}>
                                            ${cidade.nomeCidade} 
                                        </option>
                                    </c:forEach>
                                </select>
...
<script>
    ...
    function setDadosModal(valor) {
        ...
        var idCliente = valor;
        if (idCliente != "0") {
            //existe cliente para buscar (alteração)
            $.getJSON('ClienteCarregar', {idCliente: idCliente}, function (respostaServlet) {
    ...
    function carregarPessoa(v) {
        ...
        $(document).ready(function () {
            $.getJSON('PessoaBuscarCpfCnpj', {cpfcnpjpessoa: idM, tipopessoa: tipoPessoa}, function (respostaCliente) {
                console.log(respostaCliente);
                if (respostaCliente != null) {
                    $('#idcliente').val(respostaCliente.idCliente);
                    $('#permitelogin').val(respostaCliente.permiteLogin);
                    $('#situacao').val(respostaCliente.situacao);
                    $('#idpessoa').val(respostaCliente.idPessoa);
                    $('#nomepessoa').val(respostaCliente.nome);
                    $('#observacao').val(respostaCliente.observacao);
                    $('#login').val(respostaCliente.login);
                    var datanasc = respostaCliente.dataNascimento;
                    console.log(datanasc);
                    $('#datanascimento').val(datanasc);
                    $('#idestado').val(respostaCliente.cidade.estado.idEstado);
                    cidade = respostaCliente.cidade.idCidade;
                    BuscarCidadesPorEstado();
                    var foto = document.getElementById("foto");
                    foto.src = respostaCliente.foto;
                } else {
                    //se não encontrou cliente busca por pessoa somente
                    tipoPessoa = 'pessoa';
